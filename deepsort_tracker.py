...
# DeepSORT
DEEPSORT_MAX_AGE = 60              # скільки кадрів тримаємо втрачений трек
DEEPSORT_N_INIT = 3                # скільки детекцій потрібно для підтвердження треку
DEEPSORT_MAX_IOU_DISTANCE = 0.7    # поріг IoU при асоціації
DEEPSORT_MAX_COSINE_DISTANCE = 0.2 # поріг косинусної відстані embeddings
DEEPSORT_NN_BUDGET = 100           # розмір галереї embeddings на трек
DEEPSORT_EMBEDDER = "mobilenet"    # "mobilenet" | "clip_RN50" | "torchreid"
DEEPSORT_HALF = True               # FP16 для embedder на GPU

# Калібрування (px -> метри)
...
MIN_TRACK_LENGTH = 8
SPEED_SMOOTHING = 0.6
SPEED_WINDOW_POINTS = 16
DIR_MIN_DISP_PX = 3.0
MAX_LOST_SECONDS = 3

# Параметри фільтрації шуму швидкості
ANCHOR_SMOOTH_ALPHA = 0.5
STATIC_NOISE_BBOX_FRAC = 0.05
SPEED_MIN_KMH = 2.0
SPEED_ZERO_SNAP_KMH = 0.3
...
class Tracker:
    def __init__(self, fps=30):
        ...
        self.tracker = DeepSort(
            ...
            half=DEEPSORT_HALF and use_gpu,
            embedder_gpu=use_gpu,
            bgr=True,
        )
        print(f"[Tracker] DeepSORT, embedder={DEEPSORT_EMBEDDER}, gpu={use_gpu}")

    def update(self, dets, frame=None):
        """
        dets: ndarray [[x1,y1,x2,y2,conf,cls], ...]
        frame: BGR-кадр (потрібен DeepSORT для embeddings)
        """
        if frame is None:
            print("[Tracker] DeepSORT потребує frame")
            return []
        ...
        if dets is not None and len(dets) > 0:
            for d in dets:
                ...
                if w <= 0 or h <= 0:
                    continue
                raw.append(([float(x1), float(y1), w, h], float(conf), int(cls)))
        ...
        for t in tracks:
            ...
            l, tp, r, b = t.to_ltrb()
            cls = t.get_det_class()
            cls = int(cls) if cls is not None else 2

            out.append({
                "track_id": tid,
                "bbox": [float(l), float(tp), float(r), float(b)],
                "class_id": cls,
            })
        return out
...
def draw_hud(frame, frame_idx, fps_real, n_tracks, n_occluded):
    ...
    text = (f"DeepSORT | frame {frame_idx} | FPS {fps_real:4.1f} | "
...
def main():
    ...
    try:
        while True:
            ...
            tracks = tracker.update(dets, frame=frame)  # <-- єдина зміна
            ...
            if SHOW_WINDOW:
                cv2.imshow("DeepSORT - Speed Tracker", frame)
                cv2.waitKey(1)
    finally:
        cap.release()
        writer.release()
